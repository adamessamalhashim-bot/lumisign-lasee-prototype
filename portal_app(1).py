...
if is_admin:
    admin_pages = {
        "التجربة والتحليل": analysis_page,
        "لوحة الإدارة": admin_dashboard,
        "تسجيل المراجع": admin_references,
        "جمع البيانات": admin_dataset,
        "تدريب النموذج": admin_training,
        "الحوكمة والأدلة": admin_governance,
    }
    ...
    # Render only the selected page. Streamlit tabs execute every tab on each
    # rerun, which can exhaust the small Streamlit Cloud instance while a
    # video is decoded and MediaPipe is extracting landmarks.
